// 存放英文键盘的滑动动作数据。
//
// 26 键英文：q~m 每个键一个上划（带角标），下划只有 z 一个 —— Tab。
// 手机自带键盘打不出 Tab，所以把它挂到 z 的下划；其余键下划留白。
//
...
//   第三行 Z@ X* C` V= B[ N& M?   （z 的下划是 Tab）
//
// 角标位置见 shared/styles/center.libsonnet 的『英文上划/下划文字偏移』
// （键帽四角：上划右上、下划左下）。
...
local genSwipeenData(deviceType) = {  // 英文键盘与通用配置不同的同字母滑动动作，在此覆盖通用数据。
  ...
  // 下划：z 单独给了 Tab（手机键盘打不出 Tab，所以挂到 z 的下划），
  // 其余键的下划暂不显示，键帽下半区留白。
  swipe_down: {
