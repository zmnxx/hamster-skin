...
local getToolBar(theme, overrides={}) =
...

  {
    ...
    // 开启背景贴图时，预编辑区不再铺固定图：iOS 26/27 的系统键盘背板
    // 在这里负责顶部圆角。若继续叠加 preedit_back，固定的绿蓝贴图会在
    // 系统圆角外沿形成第二条弧线，出现截图中的深色边和颜色溢出。
    preeditBackgroundStyle: styleFactories.makeGeometryStyle(color[theme]['键盘背景颜色']),
