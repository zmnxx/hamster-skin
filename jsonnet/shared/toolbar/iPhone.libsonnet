...
local getToolBar(theme, overrides={}) =
...

  {
    ...
    // 胶囊背景：左右各内缩 10pt（原 0），上下各 3pt，圆角取剩余高度的一半。
    //
    // 左右内缩是为了避开系统键盘背板的圆角。元书的键盘区铺在系统背板
    // （一个圆角容器）里，内嵌模式下 preedit 高度按 0 算，工具栏就顶到了
    // 背板最上沿；胶囊两端是半圆，若从左缘 0 起画，左端半圆正好落在背板的
    // 左上圆角弧线上：
    //   · iOS 26 背板圆角小（约 13pt），胶囊半圆还在角内，看着正常；
    //   · iOS 27 背板圆角变大（约 20pt 起），胶囊半圆就跨到弧线外侧，
    //     与系统圆角交叉、局部像"溢出"了一块，背板高度在变时还会忽好忽坏。
    // 收到 10pt 后，胶囊左端最靠外的点（x=10，y=21）距背板圆角弧线仍有
    // ~2.5pt 余量，圆角一路大到 32pt（iPhone）/ 40pt（iPad）都不会压线。
    // 只收上下没用：横向内缩才是关键。
    //
    // 注意：内缩写在**胶囊背景节点**上，而不是 toolbarStyle.insets —— 后者
    // 会连按钮内容一起收，首格「菜单」文字就会顶出胶囊。胶囊只被 toolbarStyle
    // 引用，收它自己不影响任何按钮位置。
