...
local getToolBar(theme, overrides={}) =
...

  {
    ...
    preeditBackgroundStyle:
      if bgImage != null then styleFactories.makeFileImageStyle(bgImage.preedit)
    ...
    // 的边界，不会产生横贯全屏的硬边。
    //
    // 开启背景贴图后改走 toolbar_back：胶囊已经画进那张图里。
    // 当前 Custom 里 background_image 是关闭的，走的是下面的 geometry 胶囊。
    // 原因是一个区域只有一个背景槽 —— 槽给了胶囊，胶囊圆角外那圈就没人画，
    // 露出系统背板（深色 App 里近黑，与背景图接不上）。把胶囊烘进图里，
    // 一个槽同时得到「背景 + 胶囊」。
    toolbarStyle: {
      // 左右内缩与功能行、字母区统一为 4pt：三块（工具栏 / 功能行 / 字母键）
      // 共用同一条左右边界，8 个工具栏按钮与下面 8 个功能键逐列对齐。
      insets: { left: 4, right: 4 },
      backgroundStyle: if bgImage != null then 'toolbarImageBackgroundStyle'
                       else 'toolbarCapsuleBackgroundStyle',
    },
    [if bgImage != null then 'toolbarImageBackgroundStyle']:
      styleFactories.makeFileImageStyle(bgImage.toolbar),
    // 胶囊背景：左右各内缩 5.5pt，上下各 3pt，圆角取剩余高度的一半。
    //
    // 内缩是为了避开系统键盘背板的圆角。元书的键盘区铺在系统背板（一个圆角
    // 容器）里，内嵌模式下 preedit 高度按 0 算，工具栏就顶到了背板最上沿；
    // 胶囊两端是半圆，若从左缘 0 起画，左端半圆正好落在背板的左上圆角弧线上：
    //   · iOS 26 背板圆角小（约 13pt），胶囊半圆还在角内，看着正常；
    //   · iOS 27 背板圆角变大（实测 26~27pt），胶囊半圆就跨到弧线外侧，
    //     与系统圆角交叉、局部像"溢出"了一块，背板高度在变时还会忽好忽坏。
    //
    // 为什么是 5.5pt 而不是更大：胶囊两端是**半圆**，弧线在按钮文字的竖直
    // 范围内会往内收。左边第一个按钮「菜单」的文字左缘实测在 12pt，胶囊左端
    // 若收到 10pt，弧线在文字上沿处已经收进到 13.7pt，文字左上角就贴上了圆弧
    // （看着像被切到）；同时胶囊比下面功能行（可见左缘约 6.5pt）窄了一大截，
    // 整条控件显得又短又窄。5.5pt 时：
    //   · 文字左上角离弧线还有约 6.2pt；
    //   · 胶囊左端与功能行基本齐平（差约 1pt，视觉上是一条线）；
    //   · 对 27pt 的背板圆角仍留约 3.3pt 余量，背板圆角到 32pt 都还够。
    toolbarCapsuleBackgroundStyle: keycap.toolbarCapsuleBackground(
      theme,
      Settings,
      (Settings.toolbar_config.toolbar_height - 6) / 2,
      { top: 3, bottom: 3, left: 5.5, right: 5.5 }
    ),
    ...
    // 横向候选文字栏调式
    //
    // 它与工具栏同位置、出现时工具栏隐藏，是一个**独立的背景槽**。
    // 原本指向透明的 toolbarBackgroundStyle，于是打字时这一条露出系统背板
    // （深色 App 里近黑）。开启背景贴图后给它同位置的那张图，
    // 与工具栏那张取自源图的同一条带，切换时颜色不跳。
    horizontalCandidatesStyle: {
      insets: { left: 5, right: 10, },
      backgroundStyle: if bgImage != null then 'candidatesImageBackgroundStyle'
                       else 'toolbarBackgroundStyle',
    },
    [if bgImage != null then 'candidatesImageBackgroundStyle']:
      styleFactories.makeFileImageStyle(bgImage.candidates),
    ...
    // 纵向候选文字栏调式
    //
    // 它占「工具栏 + 按键区」两段的高度，出现时其余区域全隐藏，
    // 同样是独立的背景槽。原本透明会露出系统背板。
    verticalCandidatesStyle: {
      insets: { left: 3, bottom: 1, top: 3 },
      backgroundStyle: if bgImage != null then 'vcandidatesImageBackgroundStyle'
                       else 'toolbarFlatBackgroundStyle',
    },
    [if bgImage != null then 'vcandidatesImageBackgroundStyle']:
      styleFactories.makeFileImageStyle(bgImage.vcandidates),
