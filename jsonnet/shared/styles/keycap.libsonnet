...
// 浅色下键帽是半透明设计，靠底下的键盘背景透出层次，因此颜色都带 alpha；
// 深色下已压平成不透明（见 dark 段的说明）。
// ---------------------------------------------------------------------------
// 统一强调色（柔和不刺眼的青绿色渐变），回车键与长按选中块共用。
local accentDark = ['#5BAEA3', '#2E8076'];
local accentLight = ['#1E8C80', '#0E6A5F'];
...

{
  ...
  // 全部取自 gradientPalette 令牌，跟 buttonBackground() 的字母键分支一致。
