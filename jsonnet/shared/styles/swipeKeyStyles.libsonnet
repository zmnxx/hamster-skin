...
local swipeStyle(center, colorMap, fontSizeConfig, fs=null) = {
  ...
  // 英文 26 键角标专用：改用中性灰（英文划动字符颜色），
  // 与字母本体同色系，不再是棕金色。
  local enSwipeColor =
    if std.objectHas(colorMap, '英文划动字符颜色') then colorMap['英文划动字符颜色']
    else colorMap['划动字符颜色'],
  local makeEnSwipeTextStyle(fontSizeValue) =
    {
      buttonStyleType: 'text',
      fontSize: fontSizeValue,
      normalColor: enSwipeColor,
      highlightColor: enSwipeColor,
      center: center,
    },
  local makeEnSwipeSystemImageStyle(fontSizeValue) =
    {
      buttonStyleType: 'systemImage',
      fontSize: fontSizeValue,
      normalColor: enSwipeColor,
  ...
  // 英文角标样式：字号更小，改用中性灰（英文划动字符颜色），与字母本体同色系
  '英文上划样式': {
    '文字样式': makeEnSwipeTextStyle(if fs == null then fontSizeConfig['英文划动角标文字大小'] else fs),
    'sf符号样式': makeEnSwipeSystemImageStyle(if fs == null then fontSizeConfig['英文划动角标文字大小'] else fs),
  },
  '英文下划样式': {
    '文字样式': makeEnSwipeTextStyle(if fs == null then fontSizeConfig['英文划动角标文字大小'] else fs),
    'sf符号样式': makeEnSwipeSystemImageStyle(if fs == null then fontSizeConfig['英文划动角标文字大小'] else fs),
...
{
  // 划动前景样式只在划动项带 label 时才生成（见 directionalForeground）。
  // 按键接线必须用这个判断，光看 std.objectHas(swipeMap, key) 会引用到
  // 根本没生成的样式名 —— 元书对找不到的样式名静默跳过，问题不会报错。
  hasForeground(swipeMap, key)::
    std.objectHas(swipeMap, key) && std.objectHas(swipeMap[key], 'label'),
