...
// 辅助文字色：候选栏 / 划动角标 / 长按面板符号共用一支深绿。
// 分两个明度档，因为它们的底完全不同：
//   accent_soft    画在系统背板上（候选栏）——取中间调，深浅两种 App 都够看
//   accent_soft_hi 画在深色键帽 / 深色面板上（划动角标、长按符号）——提亮一档，
//                  8pt 的小角标在 #414144 的键帽上才提得起来（约 4.2:1）
local accent_soft = '2E8B70';
local accent_soft_hi = '5CB894';
...
local base_dark = {
  ...
  '长按选中字体颜色': 'FFFFFF',
  // 长按弹面板里的符号字。面板底是 #2C2C2E（深色），所以用提亮那档绿，
  // 与键帽上的划动角标同一个色；选中项坐在金色块上，保持白色不变。
  '长按非选中字体颜色': accent_soft_hi,
  '长按背景阴影颜色': '00000050',

  '候选字体选中字体颜色': accent,
  // 未选中的候选字（含序号与注释）直接画在系统背板上，没有自己的底，
  // 所以用中间调那档深绿：浅色 App 的背板上不发灰、深色 App 的背板上也够看。
  // 原来的 C2B49A 是专为深色底挑的浅棕，放在浅色背板上几乎看不见。
  '候选字体未选中字体颜色': accent_soft,
  '选中候选背景颜色': '#3A3228',
  ...
  // 中文九键 / 数字键盘的上下划标签（画在深色键帽上）
  '划动字符颜色': accent_soft_hi,
  // 英文 26 键的上划角标，与上面同一个颜色
  '英文划动字符颜色': accent_soft_hi,
  '按下气泡文字颜色': 'E6E6E6',
