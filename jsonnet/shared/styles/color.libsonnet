...
local base_light = {
  ...
  // 通长胶囊底（浅色分支）。force_single_theme='dark' 时用不到这套值，
  // 但令牌必须存在，否则改回跟随系统时这里会缺键直接编译失败。
  'toolbar胶囊背景颜色': 'FFFFFFCC',
  // 26键上下划动文字用青绿色
  '划动字符颜色': teal_light,
  '按下气泡文字颜色': '2E2E2E',
...
local base_dark = {
  ...
  'toolbar按键颜色': 'E5E5E5',
  // 工具栏按钮本身回到透明——整条工具栏改用一条通长胶囊托底
  // （见「toolbar胶囊背景颜色」），按钮各自带底会变成八个小方框，很碎。
  // 按下反馈仍要有，所以 highlight 保留一层淡色。
  'toolbar按键背景颜色-普通': '00000000',
  'toolbar按键背景颜色-高亮': '5A5A5E80',
  // 通长胶囊底。取字母键渐变的上端色再抬一点，让它在系统背板上能读出
  // 「一条控件」而不是一块色块；圆角走高度的一半（在 toolbar/iPhone.libsonnet 里给）。
  'toolbar胶囊背景颜色': '48484B',
  // 26键上下划动文字用青绿色
  '划动字符颜色': teal,
  '按下气泡文字颜色': 'E6E6E6',
