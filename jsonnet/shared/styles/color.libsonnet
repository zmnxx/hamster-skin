...
// 辅助文字的统一色（候选栏 / 划动角标 / 长按提示）：中间调青灰。
// 这三类文字没有自己的底，可能画在浅色 App 的系统背板上、也可能画在深色键帽或
// 深色面板上，所以取「对深浅两种底都够看」的明度；蓝绿底子与旧皮肤的孔雀绿呼应。
local accent_soft = '7E8C8B';
...
local base_dark = {
  ...
  '长按选中字体颜色': 'FFFFFF',
  // 长按弹面板里的符号字，与上面的候选字 / 划动角标同一个辅助色。
  // 面板底是 #2C2C2E，青灰在中大字号上仍有 3.8:1，够看；
  // 选中项坐在金色块上，保持白色不变。
  '长按非选中字体颜色': accent_soft,
  '长按背景阴影颜色': '00000050',

  '候选字体选中字体颜色': accent,
  // 候选栏、划动角标、长按提示这三类「辅助文字」统一用一个颜色（变量 accent_soft）。
  // 它们各自的底不一样（系统背板 / 深色键帽 / 深色面板），所以取中间调青灰：
  // 在浅色 App 的背板上不发灰、在深色键帽与面板上也不糊。
  // 原来的 C2B49A 与 C9A86A 是专为深色底挑的暖色，放在浅色背板上几乎看不见。
