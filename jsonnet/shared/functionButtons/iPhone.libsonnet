...
local createNotification(key, actionMap, keyboardType) = {
  ...
  // 中文键盘打字态最左侧键是重输，长按没有连续动作；其余键沿用连发表。
  [if key == 'left' && keyboardType != 'alphabetic' then 'repeatAction']: null,
  [if !(key == 'left' && keyboardType != 'alphabetic') && std.objectHas(specs.notificationRepeatMap, key) then 'repeatAction']:
  ...
  makeFunctionButtons(orientation, keyboardLayout, keyboard_type)::
    ...
    std.foldl(
      function(acc, key) acc {
        ...
        [key + 'ButtonPreeditNotification']: createNotification(key, notificationActionMap, keyboard_type),
      },
      resolvedOrderedKeys,
      {}
    ),
