...

{
  ...
  makeFunctionButtons(orientation, keyboardLayout, keyboard_type)::
    ...
    local resolvedNotificationActionMap =
      if keyboard_type == 'pinyin26' then
        notificationActionMap {
          left: { action: { shortcut: '#重输' } },
        }
      else
        notificationActionMap;
    std.foldl(
      function(acc, key) acc {
        ...
        [key + 'ButtonPreeditNotification']: createNotification(key, resolvedNotificationActionMap),
      },
      resolvedOrderedKeys,
      {}
    ),
