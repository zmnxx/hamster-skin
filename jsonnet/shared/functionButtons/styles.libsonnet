...
// 中文键盘最左侧键是重输；英文键盘没有拼写区，仍显示原来的箭头。
...
local alphabeticPreeditTextMap = preeditTextMap {
  left: '⇠',
};

{
  genFuncKeyStyles(fontSize, color, theme, center, keyboardType='pinyin')::
    // 字重与工具栏统一为 medium，否则上下两排看起来像两套字。
    // 字号 / 偏移已在 fontSize / center 层与工具栏对齐。
    // 只有英文键盘打字态没有拼写区，最左侧键保持原来的箭头；其余键盘显示「重输」。
    local typingTextMap = if keyboardType == 'alphabetic' then alphabeticPreeditTextMap else preeditTextMap;
    ...
    make(typingTextMap, 'ButtonPreeditForegroundStyle') +
