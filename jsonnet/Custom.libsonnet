{
  ...
  // 背景贴图。开启后按键区与 preedit 区铺一张固定的背景图，
  // 不再透出 iOS 系统键盘背板 —— 好处是任何 App 里观感完全一致，
  // 代价是不再随 App 变化。false 时回到透明背景（透出系统背板）。
  //
  // 图放在 <light|dark>/resources/ 下，每张图配一个同名 .yaml 描述子图区域。
  // 只铺这两段：toolbar 那一段的背景槽被工具栏胶囊占用（一个槽只能放一个
  // 样式），而键盘外轮廓的四个圆角不在这两段范围内 —— 顶部圆角落在 preedit
  // 之上、底部在系统那条地球/麦克风区域之下，所以图不需要切圆角。
  background_image: {
    enabled: true,
    keyboard: 'kb_back',      // 按键区（所有键缝所在的大面积区域）
