{
  ...
  //
  // 音量：元书没有音量参数，只能改音频文件本身。改样本值是安全的
  // （只重写 SSND 块的样本字节、保持块表与文件长度不变），
  // 但用编码器重新封装会重建块表，容易连播放都不播。
  key_sound: {
    input: 'key_click.aiff',
    delete: 'key_click.aiff',
    system: 'key_click.aiff',
  },
