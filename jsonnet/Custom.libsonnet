{
  ...
  //
  // 顶部两角按 iOS 26/27 的键盘容器形状做 alpha 蒙版，圆角不是
  // YAML 里的按键 cornerRadius：元书的 fileImage 区域本身没有容器裁切字段。
  // 圆角半径按这个包的 iPhone 竖屏实测约 20.7pt 换算到资源像素，且在
  // preedit / toolbar / candidates / vcandidates 四张可能位于最上方的图上同步处理。
  // 这不是 Apple 对所有设备公布的固定常量；Apple 的 ConcentricRectangle 文档
  // 说明系统会按外层容器动态计算圆角，所以最终边缘仍需在真机上确认。
  //
