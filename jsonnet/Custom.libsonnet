{
  ...
  //
  // 音量：元书没有音量参数，只能改音频文件本身。
  // 可行做法是「就地改样本」——只重写 SSND 块里的样本字节，长度不变，
  // FVER / COMM / CHAN / APPL 等块原封不动。元书对容器块结构挑食，
  // 用编码器重新封装（重建块表、丢掉原有块）会导致不播；只动样本不动结构就没事。
  // 本皮肤的四个音效都在来源音量上统一压了 6 dB（振幅 ×0.501）。
  key_sound: {
    // 输入键：厚实的一声「嗒」
    input: 'key_input.aif',
    // 删除键：保留本皮肤原有的清脆删除音
    delete: 'ios_delete.aiff',
    // 系统键（回车 / shift / 切键盘等）：比输入键更闷一点
    system: 'key_system.aif',
    actions: [
      // 空格单独一档，比其他系统键更松、更长
      { action: 'space', url: 'key_space.aif' },
    ],
  },
