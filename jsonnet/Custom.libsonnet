{
  ...
  //
  // 顶部圆角交给 iOS 26/27 系统背板：预编辑区不再叠加固定背景图，
  // 避免在系统弧线外沿出现第二层深色边和绿蓝颜色溢出。
  // toolbar / candidates / vcandidates 三张资源仍使用统一外层 alpha 蒙版，
  // 以便工具栏与候选状态切换时边缘保持一致。
  // 圆角不是 YAML 里的按键 cornerRadius：元书的 fileImage 区域本身没有容器裁切字段。
  // Apple 的 ConcentricRectangle 文档说明系统会按外层容器动态计算圆角，
  // 所以不同机型、方向和 App 的实际弧度仍需在真机上确认。
  //
