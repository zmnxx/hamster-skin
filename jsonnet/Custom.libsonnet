{
  ...
  //
  // 当前**关闭**，改用系统背板。原因（iOS 27 起）：
  //   系统键盘背板的顶部圆角半径由 iOS 26 的约 20.7pt 涨到约 30pt，而贴图是
  //   矩形满铺的、顶边渐隐只做到 15pt（preedit 区的高度）。于是深度
  //   15~30pt 那一段弧区里图仍是不透明的，皮肤颜色被画到系统圆角之外 ——
  //   顶部那个看着像「直角 / 颜色溢出」的小方块就是这么来的。
  //   皮肤读不到系统圆角半径（元书没有这类 conditionKey），靠 alpha 蒙版去追
  //   半径的话，系统每改一次圆角就要重调一次；而且 iPad 竖屏工具栏 57pt 与
  //   iPhone 42pt 共用同一张图，同一份蒙版在两边换算出的实际半径还不一样。
  //   所以干脆不铺图：露出的系统背板自带正确圆角，四角与接缝一并消失，
  //   iPhone / iPad / 横竖屏自动一致，系统以后再改圆角也不会复发。
  //
  // 注意这不是「跟随系统配色」：键帽、光标、符号列用的都是不透明暗色
  // （见 styles/keycap.libsonnet 的 gradientPalette 与 force_single_theme），
  // 跟着 App 变的只有「键盘底色」这一层 —— 底交给系统是这一步的目的。
  //
  // 要回到铺图方案时：enabled 改回 true，并把 5 张图（kb_back / preedit_back /
  // toolbar_back / cand_back / vcand_back，各含同名 .yaml）放回
  // <light|dark>/resources/ 下。图已从本包删除。
  // 区域背景槽的说明（每个区域只有一个槽）见 shared/toolbar/iPhone.libsonnet。
  background_image: {
    enabled: false,
