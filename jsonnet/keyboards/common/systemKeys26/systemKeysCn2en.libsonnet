// 定义中英切换键及其长按提示样式。
//
// 长按功能选择菜单（简繁 / 中英标点 / 超级提示 / 缩写 / 拆分开关）已按需求从
// 「中英切换键」上移除：cn2enButton 不再挂 hintSymbolsStyle（用 :: 隐藏），
// 长按不弹菜单，只保留点按「切到英文键盘」。
// 下面的 cn2enButtonHintSymbolsStyle* 样式定义仍全部保留 —— 九键 pinyin9
// 通过 p26Layout.cn2enButtonHintSymbolsStyleOf* 复用它们来渲染「提示灯泡键」
// 的功能面板；删掉会导致九键编译报缺字段。
{
  build(theme, orientation, keyboardLayout, Settings, color, fontSize, center, createButton, baseHintStyles):: {
    cn2enButton: createButton(
      ...
      // 隐藏长按菜单：createButton 会按 root 是否含 cn2enButtonHintSymbolsStyle
      // 自动挂上 hintSymbolsStyle，这里用 :: 改为隐藏字段，编译产物里不再出现，
      // 长按即不弹功能选择菜单。样式本身仍在下方定义，供九键复用。
      hintSymbolsStyle:: null,
      notification: [
        if Settings.keyboard_layout == 26 then 'cn2enButtonPreeditNotification' else 'spaceRightButtonPreeditNotification',
      ],
    },
    cn2enButtonPreeditNotification: {
      notificationType: 'preeditChanged',
      backgroundStyle: 'systemButtonBackgroundStyle',
      foregroundStyle: 'cn2enButtonPreeditForegroundStyle',
      action: { shortcut: '#重输' },
    },
    cn2enButtonPreeditForegroundStyle: {
      buttonStyleType: 'text',
      text: '重输',
      normalColor: color[theme]['按键前景颜色'],
      highlightColor: color[theme]['按键前景颜色'],
      fontSize: fontSize['按键前景文字大小'] - 7,
