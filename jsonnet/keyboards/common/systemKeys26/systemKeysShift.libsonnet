...

{
  build(theme, orientation, keyboardLayout, Settings, color, fontSize, createButton, baseHintStyles)::
    ...
    shiftButtonPreeditNotification: {
      ...
      action: if Settings.keyboard_layout == 26 then { shortcut: '#重输' } else Settings.shift_config.preedit_action,
    ...
    shiftButtonPreeditForegroundStyle:
      if Settings.keyboard_layout == 26 then
        ...
        makeShiftForegroundStyle(if Settings.shift_config.preedit_sf_symbol != '' then Settings.shift_config.preedit_sf_symbol else if Settings.fix_sf_symbol then 'paragraphsign' else 'inset.filled.lefthalf.arrow.left.rectangle'),
    // Shift 状态前景
