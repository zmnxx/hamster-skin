...

{
  build(theme, orientation, keyboardLayout, Settings, color, fontSize, center, createButton, baseHintStyles)::
    ...
    spaceLeftButton: createButton(
      ...
      action: if Settings.keyboard_layout == 26 then { character: '，' } else { character: '.' },
      [if Settings.keyboard_layout == 26 then 'swipeUpAction']: { character: '.' },
      [if Settings.keyboard_layout == 26 then 'swipeDownAction']: { character: '，' },
      [if Settings.keyboard_layout != 26 then 'swipeDownAction']: { character: ',' },
      // 26 键键面显示与点按、上划、下划实际输出保持一致。
    },
    spaceLeftButtonForegroundStyle:
      // 生成左侧逗号前景。
      makeTextForegroundStyle(if Settings.keyboard_layout == 26 then '，' else ',', color[theme]['按键前景颜色'], 0, { x: 0.5, y: 0.5 }),
    spaceLeftButtonForegroundStyle2:
      // 生成左侧句号前景。
