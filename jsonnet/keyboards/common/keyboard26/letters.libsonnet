...
{
  // 26 键字母表（不含分号）。27 键的完整表通过 getLetters(true) 取。
  letters: letters26,
  ...
  resolveTemplate(orientation, sizeBlock, template)::
    ...
    if template == 'normal' then { size: normalSize, bounds: {} }
      ...
      // 横屏下 t 键与 y 键对称：都占 200/784，靠 bounds 把可视区收窄到 146
      // 并各自贴左 / 贴右。原来这里用 normalSize（146），于是左列首行
      // q~t 合计只有 730/784，比其他行窄 6.9%，右边会空出一条缝。
      size: if isPortrait then normalSize else sizeBlock['t键size和bounds'].size,
