...

{
  getKeyboardLayout(theme, Settings=null)::
    ...
    local bgImage =
      if Settings != null && std.objectHas(Settings, 'background_image')
         && Settings.background_image.enabled
      then Settings.background_image else null;
    local makeKeyboardBackgroundStyle() =
      // 生成键盘区域背景。开启背景贴图时铺整片图，否则刷单色。
      if bgImage != null then styleFactories.makeFileImageStyle(bgImage.keyboard)
