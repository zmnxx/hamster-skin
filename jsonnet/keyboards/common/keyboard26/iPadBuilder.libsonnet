...

{
  ...
  getOverrides(theme, keyboardLayout, createButtonFunc, hintRoot):: (
      ...
      };
    {
      ...
      rightShiftButtonPreeditForegroundStyle: styleFactories.makeTextStyle(
        '重输',
        fontSize['按键前景文字大小'] - 3,
        color[theme]['按键前景颜色'],
        color[theme]['按键前景颜色'],
  ...
  build(config, theme, orientation, keyboardLayout)::
    ...
    local ipadOverrides =
      ...
      // 横屏沿用竖屏间距，不引入额外差异。
      {
        ...
        enterButtonBlueBackgroundStyle: keycap.buttonBackground(theme, 'portrait', Settings, color, 'enter键背景(强调色)', '功能键背景颜色-高亮'),
      } +
