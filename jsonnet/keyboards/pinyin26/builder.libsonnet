...

{
  ...
  build(context, keyboardLayout)::
    ...
    local pinyin26SwipeDown = {
      [key]: rawSwipeDown[key]
      for key in std.objectFields(rawSwipeDown)
      if !std.member(['x', 'c', 'v', 'b', 'n', 'm'], key)
    } + {
      z: { action: 'tab', label: { text: '⇥' } },
    };
    local swipeUp = pinyin26SwipeUp;
    local swipeDown = pinyin26SwipeDown;
    ...
    local hintData =
      if swipeAssistMode == 'up' then
        self.extendHintDataForSwipeAssist(hintSymbolsData.pinyin, [swipeUp], letter26KeysSpecs.letters, swipeAssistMode)
      else if swipeAssistMode == 'down' then
        self.extendHintDataForSwipeAssist(hintSymbolsData.pinyin, [swipeDown], letter26KeysSpecs.letters, swipeAssistMode)
      else if swipeAssistMode == 'all' then
        self.extendHintDataForSwipeAssist(hintSymbolsData.pinyin, [swipeUp, swipeDown], letter26KeysSpecs.letters, swipeAssistMode)
      else
    ...
    local createButton = self.createButtonFactory(context, swipeUp, swipeDown, letter26KeysSpecs.getLetters(includeSemicolon), swipeAssistMode, swipeUp, swipeDown);
    ...
    functionButtonStyles.genFuncKeyStyles(fontSize, color, theme, center, 'pinyin26') +
    slideButtonStyles.slideButtonStyles(theme) +
    functions.makeFunctionButtons(orientation, keyboardLayout, 'pinyin26') +
    ...
    extra27HintStyles +
    (if swipeAssistMode != 'none' then
       self.swipeAssistNotifications(letterSpecs, settings, swipeUp, swipeDown, swipeAssistMode)
     else
       {}) +
