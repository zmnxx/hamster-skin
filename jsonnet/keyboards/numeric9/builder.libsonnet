...

{
  ...
  build(context, layoutSpec)::
    ...
    (if useHintSymbols then hintSymbolsStyles.getStyle(theme, symbolButtonHelper.hintData) else {}) +
    {
      ...
      // keyboardBackgroundStyle 曾指向一张皮肤里不存在的 bg.png，元书遇到
      // 缺图会退回系统默认底色，数字键盘背景于是比其他键盘浅一档。
      // 现在的贴图是真实存在的（<light|dark>/resources/ 下有同名 png + yaml）。
      keyboardBackgroundStyle:
        if std.objectHas(context.Settings, 'background_image')
           && context.Settings.background_image.enabled
        then styleFactories.makeFileImageStyle(context.Settings.background_image.keyboard)
