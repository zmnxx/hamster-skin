...
// 按键音。整个皮肤只有 config.yaml 这一处声明，所以自然是全局统一的：
// 所有键盘、横竖屏、iPhone / iPad 共用同一套音效。
// 配置在 Custom.libsonnet 的 key_sound，没配就不写这个键（元书用系统音）。
local keySound =
  if std.objectHas(Settings, 'key_sound') && std.length(Settings.key_sound) > 0
  then { keySound: Settings.key_sound }
  else {};
...
local config = {
  ...
  // 数字九宫格。26 键全拼布局下用对调过返回键与符号键的那一份，
  // 九键布局保持原位。
  numeric: {
    iPhone: {
      portrait: if Settings.keyboard_layout == 9 then 'numeric_9_portrait' else 'numeric_26_portrait',
      landscape: if Settings.keyboard_layout == 9 then 'numeric_9_landscape' else 'numeric_26_landscape',
    },
    iPad: {
      portrait: if Settings.keyboard_layout == 9 then 'ipad_numeric_9_portrait' else 'ipad_numeric_26_portrait',
      landscape: if Settings.keyboard_layout == 9 then 'ipad_numeric_9_landscape' else 'ipad_numeric_26_landscape',
      floating: if Settings.keyboard_layout == 9 then 'numeric_9_portrait' else 'numeric_26_portrait',
    },
  },
...
// 从 26 键全拼点进 123 时用这一份：返回键与符号键对调，右手边直接能返回。
local numeric26 = numeric.layout('iPhone', true);
...
local ipad_numeric26 = ipad_numeric.layout('iPad', true);
...
render(ipad_numeric26, 'ipad_numeric_26')
