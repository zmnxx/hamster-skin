...
元书（Hamster3）中文九键皮肤。渐变键帽 + 逐帧按下动效，统一深色主题。
...
| **键帽外观**（渐变、描边、圆角、投影） | `shared/styles/keycap.libsonnet` 顶部的 `gradientPalette` 对象 |
...
| **回车键是否用强调色** | `Custom.libsonnet` 的 `enter_key_accent` |
...
- `keyboard_layout` — 中文键盘布局：`9` 九宫格（默认）/ `26` 全键 / `27` 双拼 27 键（26 键 + 分号键）
- `t9_hint_symbol_direct_output` — 九键长按符号是否直接上屏
...
- `keycap_style`
  - `'gradient'`（默认）— 上下渐变 + 半透明描边 + 底部亮边 + 光晕/涟漪动效
  - `'default'` — 单色键帽（无描边、无渐变）
  - 只影响按键本身；工具栏与功能行的**按钮内容与顺序**不受它影响，
    但它们的尺寸与配色在 v5 做过统一（见「六、修改记录」）
- `keycap_config`（仅 `gradient` 生效）
  ...
  - `apply_to_long_press_panel` — 长按符号面板是否也用渐变键帽配色

配色令牌集中在 `shared/styles/keycap.libsonnet` 顶部的 `gradientPalette` 对象，
改配色只需动这一处，五种键盘同时生效。
...
部分 App 会把 `returnKeyType` 设成「发送 / 前往 / 搜索」，默认对这些类型
套用强调色背景，导致同一个键在不同 App 里一会儿强调色一会儿灰。关掉后统一。
...
- `show_space_badge` — 空格键是否显示右下角小角标
- `space_badge_text` — 角标文字，留空则不显示内容
...
- `button_insets` — 按键间距（`keycap_style: 'gradient'` 时由 `keycap_config.insets` 接管）
- `cornerRadius` — 按键圆角（同上，`gradient` 时只影响符号键盘 collection）
...
- **上划**：q~m 全部 26 键各一个，共 26 个动作。
- **下划**：只有 z 一个 —— **Tab**（手机自带键盘打不出 Tab，所以挂到 z 下划）。
  其余键下划留白。
...
角标样式：**上划标在键帽左上角、下划标在右上角**，字号 8，字母本体仍居中。
两个角标都在上沿左右分开，并往键帽内侧收了一点，既不贴键帽描边、也不压住中间
的字母。

- 位置：`shared/styles/center.libsonnet` 的『英文上划/下划文字偏移』
- 字号：`shared/styles/fontSize.libsonnet` 的『英文划动角标文字大小』
- 中文 26 键用同一套角标位置（『上划/下划文字偏移』）；
  九键与数字键盘的上下划标记不受影响，仍是原来的上下居中样式
...
1. **渐变键帽** — 上下渐变、半透明描边、底部亮边、圆角 14、投影，
   以及三层按下动效（缩放 + 光晕 10 帧 + 涟漪 10 帧）。
   工具栏与功能行不走这套材质，仍用 `ButtonScaleAnimation`。
2. **强制单一配色** — `force_single_theme`，解决部分 App 强制浅色时皮肤变白。
3. **回车键统一配色** — `enter_key_accent`，解决不同 App 里回车一会儿蓝一会儿灰。
4. **26 键划动角标** — 上划 / 下划角标落在键帽四角（上划右上、下划左下），
   与居中的字母错开；英文 26 键下划含 Tab（z 键）。
5. **功能行尺寸统一** — 功能行原本复用各键盘的字母键背景样式，于是
...
2. **九键长按气泡无底** — 九键按键的 `hintStyle` 引用 `alphabeticHintBackgroundStyle`，
   但九键页原本没有定义它。样式名不存在时元书静默不渲染。已补上。
...
3. **候选栏配色统一到灰白系** — 原本首选是金棕（`E8A317` + `3A3520` 底）、
   其他候选是青绿，与中性灰键帽色相跨度太大。现在改为靠明度
   区分：首选纯白 + `3D3D3D` 底，其他候选 `B4B4B8`。
4. **工具栏胶囊左右内缩 4 → 5.5pt** — 修 iOS 27 内嵌模式下胶囊圆角与系统
   背板圆角重叠/溢出。起因是区域 insets 只作用于内容、背景仍铺满整宽，
   胶囊实际从 x=0 起画；它的两端是半圆，落在背板左上圆角弧线上时，
   背板圆角一大（iOS 27 实测 24~27pt）就跨出弧线。
   取值受两端夹击：太小又撞回系统圆角，太大则「菜单」二字被左端半圆贴到
   （K=10 时文字左上角只剩 1.75pt）、且比下面功能行窄一大截。
   5.5pt 时文字左上角离弧线约 6.2pt、对 27pt 背板圆角留 3.2pt 余量、
   胶囊两端与功能行基本齐平。改 `shared/toolbar/iPhone.libsonnet` 与
   `iPad.libsonnet` 里 `toolbarCapsuleBackgroundStyle` 的 `insets`。
...
- 清理 `Custom.libsonnet` 里已失效的兼容项
- 合并 README 与 MODULES 两份重复文档为本文档

源码从 85 个文件 10427 行降到 64 个文件 9015 行，编译产物不变。
