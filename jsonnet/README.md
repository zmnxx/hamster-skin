...
4. **工具栏胶囊左右内缩 4 → 10pt** — 修 iOS 27 内嵌模式下胶囊圆角与系统
   背板圆角重叠/溢出。起因是区域 insets 会同时作用于**背景图层**，胶囊
   原先从 x=4 起画、通栏；它的两端是半圆，落在背板左上圆角弧线上时，
   背板圆角一大（iOS 27）就跨出弧线。收到 10pt 后对 32pt 以内的背板圆角
   都留有余量。改 `shared/toolbar/iPhone.libsonnet` 的 `toolbarStyle.insets`。
